...
await c.query(
  `set local request.jwt.claims = '${JSON.stringify({ sub: userId, role: 'authenticated', aud: 'authenticated' })}'`,
);
...
try {
  ...
  const r = await c.query(
    `insert into public.projects (name, owner_id, template, compiler) values ('LiteralTest', '${userId}', 'article', 'tectonic') returning id, owner_id, auth.uid() as uid_at_insert`,
  );
...
const polCheck = await c.query(
  "select polname, polcmd, pg_get_expr(polqual, polrelid) as q, pg_get_expr(polwithcheck, polrelid) as wc from pg_policy where polrelid='public.projects'::regclass and polcmd='a'",
);
console.log('insert policy:', polCheck.rows);

// Check if there are any policies that might be PERMISSIVE FALSE
const allPol = await c.query(
  "select polname, polpermissive, polcmd from pg_policy where polrelid='public.projects'::regclass",
);
