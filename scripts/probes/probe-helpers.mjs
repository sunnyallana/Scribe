...
await c.connect();
const r = await c.query("select proname, prosecdef, pg_get_functiondef(oid) as src from pg_proc where proname in ('is_project_member','my_project_role','project_role','handle_new_project') and pronamespace = 'public'::regnamespace");
