...
await c.query("create policy projects_insert_self_owner on public.projects for insert with check (public.dbg_insert_check(auth.uid(), owner_id))");
...
try {
  const r = await c.query("insert into public.projects (name, owner_id, template, compiler) values ('Trace', $1::uuid, 'article', 'tectonic') returning id", [userId]);
...
await c.query("reset role");
await c.query("drop policy if exists projects_insert_self_owner on public.projects");
await c.query("create policy projects_insert_self_owner on public.projects for insert with check (auth.uid() = owner_id)");
